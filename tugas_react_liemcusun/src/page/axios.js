...
class Axios extends React.Component {
    ...
    dropDownCountry = () => {
        return this.state.countryID.map((item, index) => {
            return (
                tempCountry.push(this.state.countryID[index]),
                imgLink = `https://www.countryflags.io/${tempCountry[index]}/shiny/64.png`,
                <NavDropdown.Item onClick={() => arrValue = index}>
                    <img alt= "bendera" src={imgLink}/>  
                    {"  " + this.state.countryID[index]}</NavDropdown.Item>
            )
        })
    }
